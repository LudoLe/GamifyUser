    ...
    for (let item of listItems) {
        item.addEventListener("click", (e) => {
            ...
            $.getJSON(baseUrl + currentItem.id, function (data) {
                ...
                row.setAttribute("id", "statsRow");
                answersColumn.append(row);
                let btn1 = document.createElement("button");
                btn1.classList.add("btn", "btn-outline-primary", "btn-stats");
                row.append(btn1);
                let btn2 = document.createElement("button");
                btn2.classList.add("btn", "btn-stats", "btn-stats-md");
                row.append(btn2);
                let btn3 = document.createElement("button");
                btn3.classList.add("btn", "btn-stats", "btn-stats-md");
                row.append(btn3);
                btn1.id = "btnAge";
                btn2.id = "btnSex";
                btn3.id = "btnExp";
                ...
                divOptional.classList.add("optional-answers-scrollable");
                answersColumn.append(divOptional);
                $.each(optional, function (key, val) {
                    let card = document.createElement("div");
                    card.classList.add("card", "answers-card");
